    ...
    stages {
        ...
        stage('Docker image build and push') {
            steps {
                ...
                sh('docker build  --no-cache -t "irccs-auth:${BRANCH_NAME}:_${ARTIFACT_VER}" --build-arg folder=target .')
        ...
        stage('Build immagine Kubernetes') {
            steps {
                ...
                sh('mvn clean package -DskipTests -U')
                sh('docker build  --no-cache -t "irccs-auth_k8s:${BRANCH_NAME}_${ARTIFACT_VER}" --build-arg folder=target .')
                sh('docker login -u docker_service_user -p Infocube123 nexus.infocube.it:443')
